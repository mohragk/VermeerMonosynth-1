/*
  ==============================================================================
    ...
    Created: 1 Nov 2018 4:26:20pm
    Author:  VLSDESK003

  ==============================================================================
*/

#ifndef LEVEL_METER_PROCESSOR_H
#define LEVEL_METER_PROCESSOR_H
	...
	private:
		...
		void pushNextRMS(const float newRMS) {
			...
			if (rmsHistory.size() > 0) 
			{
				...
				rmsSum = rmsSum + squaredRMS - rmsHistory[rmsIndex];
				rmsHistory[rmsIndex] = squaredRMS;
...
public:
	...
	void decayIfNeeded()
	{
		...
		if (time - lastTime > 100) {
			...
			for (int channel = 0; channel < levels.size(); ++channel) {
				levels[channel].setLevels(lastTime, 0.0f, 0.0f, holdMillis);
	...
	*/
	...
	current peak
	*/
	void setMaxPeakHoldMillis(const juce::int64 millis)
	{
		holdMillis = millis;
	}
...
private:
    ...
    juce::WeakReference<LevelMeterProcessor>::Master masterReference;
    friend class juce::WeakReference<LevelMeterProcessor>;

    std::vector<ChannelData> levels; // make mono

    juce::int64 holdMillis;

    std::atomic<juce::int64> lastTime;
